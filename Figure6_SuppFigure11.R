#####################################################################
## Figure 6. Gene based analyses (& Supplementary Figure 11)
#####################################################################
...
#####################################################################
### Figure 6A: Differential gene analyses among genes with ART, followed by the scripts for bootstrapping analyses ================
...
TCGA.download <- lapply(c('BRCA','LUAD'), function(cancer_type){
  print(cancer_type)
...
names(TCGA.download) <- c('BRCA','LUAD')
...
## Step 3. Run bootstrapping analyses using log2.fold.change of each gene in BRCA, LUAD: ======
...
ARTgenes.consist_type_list <- lapply(cancer.types, function(ca){
  print(ca)
  # genes with shared ART or consistent ART in BRCA, LUAD (saved in data_dir)
...
TCGA.tumour_expressed.genes <- lapply(c('LUAD','BRCA'), function(cancer_type){
  print(cancer_type)
...
names(TCGA.tumour_expressed.genes) <- c('LUAD','BRCA')
...
## Make plots: DEG
cohort <- c("TCGA")
data <- boot.logFC_ART_list[['TCGA']]

cancer.types <- c('BRCA', 'LUAD')

plots.list <- lapply(data, function(x){
  # x=grep('boot.logFC_rm.ART_', list.files(output_dir), value=T)[1]
  print(x)
  input <- readRDS(paste0(output_dir, x))[[cohort]] %>%
    filter(cancer.type %in% cancer.types)
  input
  # plot mean_original / mean_iter on two columns:
  plot.boot.logFC <- input %>% 
  ...
  p_boot_by.cancer2 <- plot.boot.logFC %>% 
    ...
    # geom_text(aes(y=y.dot.value, label=paste0(numb.label,'=', n.test.genes)), 
    ...
    # facet_grid(factor(ARTgene.list, levels = test.gene.lists ) ~ factor(cancer_type, levels=cancer_types),scales="free") +
  ...
  x
  name <- sub('boot.logFC_','',x) %>% sub('.RDS', '', .)
  name
  
  pdf(paste0(output_dir, Sys.Date(), '_p.boot_logFC_', name,'.pdf'), 
      width = 9, height = 4)
...
#####################################################################
### Figure 6B: bootstrapping of CN/Ploidy ========
...
#calculated copy number difference to ploidy per gene for BRCA and LUAD from ascat data provided by TCGA
...
# Make plots:
cohort <- c("TCGA")
data <- boot.CN.ploidy_ART_list

cancer.types <- c('BRCA', 'LUAD') 

plots.list <- lapply(data, function(x){
  print(x)
  input <- readRDS(paste0(output_dir, x))
  input <- input[[cohort]] %>%
    filter(cancer.type %in% cancer.types)
  input
  # plot mean_original / mean_iter on two columns:
  ...
  plot.boot.logFC$x.label %>% unique()
  color.key <- c(brewer.pal(9,'PiYG')[c(1,2, 8,9)])
  names(color.key) <- c("early", "earlier","later","late")
  color.key
  ...
  x
  name <- sub('boot.CN.ploidy-1_','',x) %>% sub('.RDS', '', .)
  name
  
  pdf(paste0(output_dir, Sys.Date(), '_p.boot_CN.ploidy_', name,'.pdf'), 
      width = 8, height = 5)
  print(p_boot_by.cancer2)
  dev.off()
...
ARTgenes.consist_type_list <- lapply(cancer.types, function(x){
  ...
  print(x)
  dir <- './data'
  strict.or.not <- c('_strict', '')[2]
  list.files(dir) 
  data <- fread( paste0(dir, x,"_shared", "ARTgenes",strict.or.not,".txt") ) %>%
    unique
  return(data)
})
names(ARTgenes.consist_type_list) <- cancer.types
...
{
  ARTgenes.consist_df <- ARTgenes.consist_type_list[[1]] %>% 
    dplyr::select('gene_name','chr','start','stop','timing') %>% rename(LUAD = timing) #
  ...
  count_ART <- apply(ARTgenes.consist_df[,c(5:6)], 1, function(x) length(x[x%in%c('earlier','later')]))
  
  timings <- apply(ARTgenes.consist_df[,c(5:6)], 1, function(x){
    if( length(unique(x))==1 ){
      y <- unique(x)
    }else{
      y <- 'discordant'
  ...
  keep <- apply(ARTgenes.consist_df[,c(5:6)], 1, function(x) !anyNA(x) )
  genes.consistCRT <- ARTgenes.consist_df[keep, ]%>%
    filter(count_ART==0 & timing!='discordant')
}
...
shared.genes_CAG_list <- lapply(cancer.types, function(x){
  ...
  sub.shared.all <- ARTgenes.consist_type_list[[x]] %>%
    ...
    left_join(to.comb_all.geneSet,by=c('gene_name'='gene')) # load geneset (resource)
  sub.shared.all[1:2,]
...
shared.genes_CAG[1:4,]
ggdensity(shared.genes_CAG$freq)
...
# align legends
source('./fun_alignLegends_ggplot.R')

{
  ...
  # export plot:
  # g$heights[id_panels_h] <- grid::unit(c(1,4,1,1,1,1,1,1), "null")
...
pdf(paste0(output_dir,'new03_count.CAG_per.cancer','.pdf'), 
    width = 12, height = 8.5)
grid.arrange(g, aligend.legends1, ncol = 2, widths = c(9, 1.2))
dev.off()
